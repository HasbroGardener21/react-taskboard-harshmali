...
The tradeoff is vulnerability to XSS attacks — 
an httpOnly cookie would be more secure in production 
...
## Engineering Challenges & Pivots

During this sprint, I prioritized application stability, data integrity, and security. Here are the core engineering decisions made to achieve that:

* **UI Stability Pivot (Frontend):** During the Day 10-12 frontend integration, the `@material-tailwind/react` library caused severe compilation crashes and `useContext` failures due to Vite caching conflicts with React 18. Rather than shipping a broken UI, I bypassed the dependency and rebuilt the components (including the dashboard and task forms) in pure Tailwind CSS to guarantee stability.
* **State Persistence Lock (Frontend):** Identified and resolved a React state race condition where default initial state was overwriting localized user data upon browser refresh. I implemented an initialization lock within the `AppContext` to ensure seamless data persistence across sessions.
* **API Hardening & Security (Backend):** As a core requirement, the backend is strictly locked down. All admin-level endpoints are protected by RBAC (Role-Based Access Control) middleware, and the API is hardened using Helmet, CORS policies, and rate-limiting to prevent basic exploitation.
